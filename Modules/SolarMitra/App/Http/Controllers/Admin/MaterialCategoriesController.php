...
class MaterialCategoriesController extends Controller
{
    ...
    public function list(Request $request, $id='')
    {
        ...
        $material_categories    = (new MaterialCategory)->generateCategoryTreeArray(Null, "_", ['id', 'title','display_on_invoice','include_in_solar_kit','calculate_in_invoice','description', 'created_at']);
        ...
        if($request->isMethod('post'))
        {
            ...
            if($res)
            {
                ...
                $notificationObj->notification_entry($notify_code, $materialCategory->id, auth()->id(), config('constants.superadmin'));
                ...
                return redirect()->route('admin.solarmitra.material_categories.list')->with('success', $msg);
            }
            return redirect()->back()->with('error', __('solarmitra::solarmitra.something_went_wrong'));
        }
        return view('solarmitra::admin.material-categories.list',compact('page_title','material_categories','materialCategory','material_units'));
    ...
    public function destroy($id)
    {
        ...
        /* Send Event Notification */
        $notificationObj        = new Notification();
        $notificationObj->notification_entry('MATERIALCATEGORY-DMC', $materialCategory->id, auth()->id(), config('constants.superadmin'));
        /* End Send Event Notification */
