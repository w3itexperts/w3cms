        ...
        if($request->isMethod('post'))
        {
            ...
            if($res)
            {
                /*if (!$materialUnitObj->wasRecentlyCreated) {
                    ...
                    $notificationObj->notification_entry('CHANNEL-UC', $materialUnitObj->id, auth('business')->id(), config('constants.superadmin'));
                }else{
                    $notificationObj        = new Notification();
                    $notificationObj->notification_entry('CHANNEL-ANC', $materialUnitObj->id, auth('business')->id(), config('constants.superadmin'));
                }*/

                if ($materialUnitObj->exists) {
                    return redirect()->route('business.solarmitra.material_units.list')->with('success', __('solarmitra::solarmitra.material_unit_saved'));
                }
                return redirect()->back()->with('success', __('solarmitra::solarmitra.material_unit_saved'));
            }
            return redirect()->back()->with('error', __('solarmitra::solarmitra.something_went_wrong'));
        }
        return view('solarmitra::business.materials.material_units',compact('page_title','materialUnitObj','material_units'));
