...
class MaterialsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('solarmitra::business.materials.index',compact('page_title','materials'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            return view('solarmitra::business.materials.material_modal',compact('material_companies','material_categories','material_units'));
        }

        return view('solarmitra::business.materials.create',compact('material_companies','material_categories','material_units'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $messages = [
            ...
            'material_category_id.required' => __('solarmitra::solarmitra.please_select_material_category'),
            'selling_price.required' => __('solarmitra::solarmitra.selling_price_is_required'),
            'selling_price.numeric' => __('solarmitra::solarmitra.selling_price_must_be_a_number'),
            'purchase_price.required' => __('solarmitra::solarmitra.purchase_price_required'),
            'purchase_price.numeric' => __('solarmitra::solarmitra.purchase_price_must_be_a_number'),
            'material_company_id.required' => __('solarmitra::solarmitra.please_select_material_company'),
            'unit_id.required' => __('solarmitra::solarmitra.please_select_a_unit'),
            'hsn_sac.regex' => __('solarmitra::solarmitra.hsn_sac_must_be_a_valid_number_can_be_decimal'),
        ];
        ...
        if ($res) {
            ...
            $notificationObj->notification_entry('MATERIAL-ANM', $materialObj->id, auth('business')->id(), config('constants.superadmin'));
            ...
            return redirect()->route('business.solarmitra.materials.index');
        }
    ...
    public function edit(Request $request,$id)
    {
        ...
        if ($request->ajax()) {
            return view('solarmitra::business.materials.material_modal',compact('page_title','material','material_companies','material_categories','material_units'));
        }

        return view('solarmitra::business.materials.edit',compact('material','page_title','material_companies','material_categories','material_units'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $messages = [
            ...
            'unit_id.required' => __('solarmitra::solarmitra.please_select_a_unit'),
            'selling_price.required' => __('solarmitra::solarmitra.selling_price_is_required'),
            'selling_price.numeric' => __('solarmitra::solarmitra.selling_price_must_be_a_valid_number'),
            'purchase_price.required' => __('solarmitra::solarmitra.purchase_price_required'),
            'purchase_price.numeric' => __('solarmitra::solarmitra.purchase_price_must_be_a_valid_number'),
            'hsn_sac.regex' => __('solarmitra::solarmitra.hsn_sac_must_be_a_valid_number_can_include_decimals'),
        ];
        ...
        if ($res) {
            ...
            /* Send Event Notification */
            $notificationObj        = new Notification();
            $notificationObj->notification_entry('MATERIAL-UM', $materialObj->id, auth('business')->id(), config('constants.superadmin'));
            /* End Send Event Notification */

            if ($request->ajax()) {
                return response()->json(['status' => true,'reload' => true,'message' => __('solarmitra::solarmitra.material_updated_text')]);
            }
            return redirect()->route('business.solarmitra.materials.index')->with('success', __('solarmitra::solarmitra.material_updated_text'));
        }
    ...
    public function destroy($id)
    {
        ...
        /* Send Event Notification */
        $notificationObj        = new Notification();
        $notificationObj->notification_entry('MATERIAL-DM', $material->id, auth('business')->id(), config('constants.superadmin'));
        /* End Send Event Notification */
    ...
    public function export(Request $request)
    {
        /* Send Event Notification */
        $notificationObj        = new Notification();
        $notificationObj->notification_entry('MATERIAL-EM', 0, auth('business')->id(), config('constants.superadmin'));
        /* End Send Event Notification */
    ...
    {
        $page_title = __('solarmitra::solarmitra.material_import');
        ...
        if($request->isMethod('post'))
        {
            ...
            try {
                ...
                if ($failures->isNotEmpty()) {
                    ...
                    return back()->with('error', 'Some rows failed. Check details.');
                }
                
                /* Send Event Notification */
                $notificationObj        = new Notification();
                $notificationObj->notification_entry('MATERIAL-IM', 0, auth('business')->id(), config('constants.superadmin'));
                /* End Send Event Notification */

                return back()->with(
                    'success',
                    __('solarmitra::solarmitra.import_result', ['imported' => $import->importedCount, 'duplicates' => $import->duplicateCount, 'failed' => $failedCount, 'rows' => $rowNumbers])
                );
        ...
        return view('solarmitra::business.materials.import');
    }
