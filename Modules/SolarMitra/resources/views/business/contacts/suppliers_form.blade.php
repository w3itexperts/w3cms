	...
	<div class="offcanvas-body">
		...
		<form action="{{@$action}}" method="post" id="ContactForm">
			...
			<div class="row">
				...
				<!----  Supplier Table Fields Start  ----->
				<div class="col-xl-6 mb-3">
					<label class="form-label">{{ __('solarmitra::solarmitra.supplier_category') }} </label>
					<select name="supplier_category" class="form-control selectpicker text-primary">
						<option value="">{{ __('solarmitra::solarmitra.select') }} {{ __('solarmitra::solarmitra.category') }}</option>
						<option value="panels" @selected(@$contact->supplier->supplier_category == 'panels')>{{ __('solarmitra::solarmitra.panels') }}</option>
						<option value="inverter" @selected(@$contact->supplier->supplier_category == 'inverter')>{{ __('solarmitra::solarmitra.inverter') }}</option>
						<option value="battery" @selected(@$contact->supplier->supplier_category == 'battery')>{{ __('solarmitra::solarmitra.battery') }}</option>
						<option value="structure" @selected(@$contact->supplier->supplier_category == 'structure')>{{ __('solarmitra::solarmitra.structure') }}</option>
					</select>
	                <p class="text-danger error-text m-0 supplier_category_error"></p>
				</div>

				<div class="col-xl-6 mb-3">
					<label class="form-label">{{ __('solarmitra::solarmitra.brand_name') }} </label>
					<input type="text" class="form-control" name="brand_name" value="{{@$contact->supplier->brand_name}}">
	                <p class="text-danger error-text m-0 brand_name_error"></p>
				</div>
				...
				<div class="col-xl-6 mb-3">
					...
					<select name="payment_terms" class="form-control selectpicker text-primary">
						...
						<option value="credit" @selected(@$contact->supplier->payment_terms == 'credit')>{{ __('solarmitra::solarmitra.credit') }}</option>
						<option value="advance" @selected(@$contact->supplier->payment_terms == 'advance')>{{ __('solarmitra::solarmitra.advance') }}</option>
					</select>
	                <p class="text-danger error-text m-0 payment_terms_error"></p>
				</div>

				<div class="col-xl-6 mb-3">
					<label class="form-label">{{ __('solarmitra::solarmitra.delivery_time_days') }} </label>
					<input type="number" class="form-control" name="delivery_time_days" min="0" value="{{@$contact->supplier->delivery_time_days}}">
					<p class="text-danger error-text m-0 delivery_time_days_error"></p>
				</div>

				<div class="col-xl-6 mb-3">
					<label class="form-label">{{ __('solarmitra::solarmitra.service_area') }} </label>
					<input type="text" class="form-control" name="service_area" value="{{@$contact->supplier->service_area}}">
	                <p class="text-danger error-text m-0 service_area_error"></p>
				</div>

				<div class="col-xl-6 mb-3">
					<label class="form-label">{{ __('solarmitra::solarmitra.rating') }} </label>
					<input type="number" class="form-control" name="rating" min="0" value="{{@$contact->supplier->rating}}">
				...
				<div class="col-xl-6 mb-3">
					...
					<select name="status" class="form-control selectpicker text-primary">
						<option value="1" @selected(@$contact->supplier->status == '1')>{{ __('solarmitra::solarmitra.active') }}</option>
						<option value="0" @selected(@$contact->supplier->status == '0')>{{ __('solarmitra::solarmitra.inactive') }}</option>
					</select>
	                <p class="text-danger error-text m-0 status_error"></p>
				</div>
				<!----  Supplier Table Fields End  ----->
