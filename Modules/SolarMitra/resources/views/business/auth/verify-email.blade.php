...
<div class="authincation h-100">
    <div class="container h-100">
        <div class="row justify-content-center h-100 align-items-center">
            <div class="d-flex flex-column gap-3 justify-content-center align-items-center py-5 vh-100" style="min-height:480px; border-radius:16px;">
                ...
                <div class="card verify-card shadow-sm p-4 h-auto" style="width:380px;">
                    <div class="text-center mb-4">
                        <div class="icon-wrap mx-auto mb-3">
                            <i class="icon-mail-check fs-3 text-primary"></i>
                        </div>
                        <h5 class="fw-semibold mb-1">Verify your email address</h5>
                        <p class="text-muted small mb-0">We sent a 6-digit code to</p>
                        <p class="fw-medium small mb-0">{{ $user->email }}</p>
                        <a href="{{ route('business.solarmitra.auth.update_contact_form', ['type' => 'email']) }}" class="btn btn-link btn-sm p-0 text-muted small mt-1">
                    ...
                    <form action="{{ route('business.solarmitra.auth.verify_user') }}" method="POST" id="VerifyUserForm">
                        ...
                        <button type="button" class="btn btn-primary w-100" id="verify-btn" disabled>
                        Verify
                        </button>
                        ...
                        <input type="hidden" name="otp_type" value="email"> {{-- or 'mobile' --}}

                        {{-- Resend section --}}
                        <div class="text-center small text-muted mt-3">
                            Didn't receive it?
                            <button type="button" class="btn btn-link btn-sm p-0 fw-medium" id="resend-btn" disabled>
                                Resend OTP
                            </button>
...
.otp-input:focus { border-color: #0d6efd; box-shadow: 0 0 0 3px rgba(13,110,253,.15); outline: none; }
.otp-input.filled { border-color: #0d6efd; background: #f0f5ff; }
.verify-card { border-radius: 20px; border: 1px solid #e9ecef; }
.icon-wrap { width: 64px; height: 64px; border-radius: 50%; background: #e7f1ff; display: flex; align-items: center; justify-content: center; }
.btn-verify { border-radius: 10px; padding: 12px; font-weight: 500; font-size: 15px; }
</style>
@endpush
