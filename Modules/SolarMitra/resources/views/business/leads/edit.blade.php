...
<div class="container-fluid">
    ...
    <form action="{{ route('business.solarmitra.leads.update',@$lead->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="lead_added_by_id" value="{{@$lead->lead_added_by_id}}">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('solarmitra::solarmitra.edit') }} {{ __('solarmitra::solarmitra.lead') }}</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-3 mb-3">
                                ...
                                <select name="client_group_id" class="form-control  text-primary selectpicker" >
                                    ...
                                    @forelse ($client_groups as $key => $title)
                                        <option value="{{$key}}" @selected(old('client_group_id', @$lead->client_group_id) == $key)>{{$title}}</option>
                                    @empty
                                    @endforelse
                                </select>
                            ...
                            <div class="col-3 mb-3 ">
                                ...
                                <select name="repeat_followup" class="form-control  text-primary selectpicker">
                                    @forelse (config('solarmitra.repeat_followups') as $key => $title)
                                        <option value="{{$key}}" @selected(old('repeat_followup',@$lead->repeat_followup) == $key)>{{$title}}</option>
                                    @empty
                                    @endforelse
                                </select>
                            ...
                            <div class="col-3 mb-3 ">
                                ...
                                <div class=" form-switch">
                                    <input class="form-check-input" type="checkbox" name="email_opt_out" role="switch" id="email_opt_out" value="1" @checked(@$lead->email_opt_out == 1)>
                                </div>
                            </div>
                        </div>
                    </div>
