@php
	...
	$post_type_video		= $blog_options['post_type_video'] ?? null;
				...
				@if ($status == 'unlock_'.$blog->id)
					...
					<div class="{{ $blog_classes }}">
						@if (!empty($post_type_video))
							@php
								$res = 0;
								if(preg_match("/(youtube.com|youtu.be)\/(watch)?(\?v=)?(\S+)?/", $post_type_video, $res)){
									$video_id = $res[4];
								}else{
									$video_id = 0;
								}
							...
							<div class="embed-responsive embed-responsive-16by9">
								<iframe class="embed-responsive-item" src="{{  $video_link }}" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
							</div>
							</div>
						@endif
						<div class="dlab-post-info">
							...
							<div class="dlab-post-text text">
								...
								<p class="blog-excerpt fs-5">{{ DzHelper::theme_lang(optional($blog)->excerpt) }}</p>
		                        {!! $blog->content !!}
								<div class="clearfix"></div>
							</div>
