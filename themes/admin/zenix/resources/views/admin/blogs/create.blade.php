...
<div class="container-fluid">
	...
	<form action="{{ route('blog.admin.store') }}" method="post" enctype="multipart/form-data">
		...
		<div class="row">
								...
								<div class="form-group">
									...
									<select name="data[Blog][user_id]" class="default-select form-control" id="ContentUserId">
										@forelse($users as $user)
											<option value="{{ $user->id }}" {{ old('data.Blog.user_id',auth()->id()) == $user->id ? 'selected="selected"' : '' }}>{{ $user->full_name }}</option>
										@empty
										@endforelse
									</select>
								</div>
			...
			<div class="col-md-4">
				<div class="row">
		            ...
		            {!! CptHelper::cpt_categories_box('blog', $screenOption, old('data.TaxoCategory')) !!}
					<div class="col-md-12">
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XVideo {{ $screenOption['Video']['visibility'] ? '' : 'd-none' }}" id="accordion-video">
							...
							<div class="accordion__body p-4 collapse show" id="with-video" data-bs-parent="#accordion-video">
								...
								<input type="text" name="data[BlogMeta][1][value]" class="form-control " placeholder="{{ __('common.youtube_video_link') }}" id="BlogMeta1Value" value="{{ old('data.BlogMeta.1.value') }}">
							</div>
						</div>
					</div>
				</div>
		...
		{!! CustomFieldHelper::custom_fields('blogs') !!}
		{!! ThemeOption::AttachBlogOptions() !!}
	</form>
</div>
